		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="replydate" value="2023-04-05 13:23">
				</div>
			</div>
...
<script>
	$(document).ready(function () {
		...
		// replyService 사용. 댓글 등록.
		/* replyService.add({bno: 300, reply: 'reply test', replyer: 'user00'}, function(){
		  alert("Result:" + result);
	  }) */
		...
		function showList(page) {
			...
			replyService.getList({
				...
				for (var i = 0; i < list.length; i++) {
					console.log(list[i]);
					str += "<li class='left clearfix' data-rno='" + list[i].rno + "'>";//아이디값을 지정해둠 (data-rno)
		...
		$('#addReplyBtn').on('click', function (e) {
			...
			modalInputReplyDate.closest('div').hide(); // 첫 등록 시 등록일자 화면에서 숨김
		...
		$('.chat').on('click', 'li', function(e){
			...
			replyService.get(rno, function (reply) {
				...
				modalInputReplyDate.val(replyService.displayTime(reply.replydate));//날짜 포맷을 맞추기 위해서
				modal.data('rno', reply.rno); // <li data-rno=32> // list rno에 댓글 번호 넣어줌
